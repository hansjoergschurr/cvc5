  ...
  if(plane.isZero()){
    ...
    if (proofsEnabled())
    {
      ...
      Pf pfBot = d_pnm->mkNode(
          ProofRule::MACRO_SR_PRED_TRANSFORM, {pfSum}, {nm->mkConst<bool>(false)});
    ...
    if(reEq.getKind() == CONST_BOOLEAN){
      if(reEq.getConst<bool>() == isDistinct){
        ...
        if (proofsEnabled())
        {
          ...
          Pf pf = d_pnm->mkScope(d_pnm->mkNode(ProofRule::MACRO_SR_PRED_TRANSFORM,
    ...
    if (d_tableau.getRowLength(ridx) <= options().arith.arithPropAsLemmaLength)
    {
      ...
      if (isProofEnabled())
      {
        ...
        auto sumPf = d_pnm->mkNode(
            ProofRule::MACRO_ARITH_SCALE_SUM_UB, conflictPfs, farkasCoefficients);
