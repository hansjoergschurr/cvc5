    ...
    std::unordered_set<Node>& assumptions) const
{
    ...
    else
    {
      ...
      else
      {
        ...
        for (unsigned i = 0; i < 2; ++i)
        {
          ...
          {
            p->addStep(
                transConclusion1, ProofRule::TRANS, copy1ofExpPremises, {}, true);
          }
          if (copy2ofExpPremises.size() > 1
              && !assumptions.count(transConclusion2))
          {
            p->addStep(
                transConclusion2, ProofRule::TRANS, copy2ofExpPremises, {}, true);
          }
        }
      }
    }
  ...
  else
  {
    ...
    p->addStep(
        newExpansionConclusion, ProofRule::TRUE_INTRO, {expansionConclusion}, {});
    ...
    Node conclusion, std::vector<Node>& premises, CDProof* p) const
{
  ...
  p->addStep(constEquality, ProofRule::MACRO_SR_PRED_INTRO, {}, {constEquality});
