  ...
  if (atom.getKind() == kind::EQUAL && !pol && !isInternal)
  {
    ...
    if (fact[0][0].getType().isArray() && !d_state.isInConflict())
    {
          ...
          && d_equalityEngine->hasTerm(bk))
      {
        ...
        d_im.assertInference(eq, false, InferenceId::ARRAYS_EXT, fact, ProofRule::ARRAYS_EXT);
        ++d_numProp;
      }
      ...
      d_im.arrayLemma(eq.notNode(), InferenceId::ARRAYS_EXT, fact, ProofRule::ARRAYS_EXT);
      ++d_numExt;
    }
    else
    {
  ...
  if (prop > 0) {
    if (d_equalityEngine->areDisequal(i, j, true) && (bothExist || prop > 1))
    {
      ...
      d_im.assertInference(
          aj_eq_bj, true, InferenceId::ARRAYS_READ_OVER_WRITE, reason, ProofRule::ARRAYS_READ_OVER_WRITE);
      ++d_numProp;
      return;
    }
    if (bothExist && d_equalityEngine->areDisequal(aj, bj, true))
    {
      ...
      d_im.assertInference(
          j_eq_i, true, InferenceId::ARRAYS_READ_OVER_WRITE_CONTRA, reason, ProofRule::ARRAYS_READ_OVER_WRITE_CONTRA);
      ++d_numProp;
      return;
    }
  }
...
void TheoryArrays::queueRowLemma(RowLemmaType lem)
{
  ...
  if (options().arrays.arraysEagerLemmas || bothExist)
  {
    ...
    d_im.arrayLemma(
        aj.eqNode(bj), InferenceId::ARRAYS_READ_OVER_WRITE, eq2.notNode(), ProofRule::ARRAYS_READ_OVER_WRITE);
    ++d_numRow;
  }
  else {
    d_RowQueue.push(lem);
