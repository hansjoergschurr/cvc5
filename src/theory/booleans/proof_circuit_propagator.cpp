                                                             ...
                                                             Node parent)
{
  ...
  return mkNot(mkResolution(
      mkProof(y ? ProofRule::NOT_EQUIV_ELIM2 : PfRule::NOT_EQUIV_ELIM1,
                                                             ...
                                                             Node parent)
{
  ...
  return mkNot(mkResolution(
      mkProof(x ? ProofRule::NOT_EQUIV_ELIM2 : PfRule::NOT_EQUIV_ELIM1,
              {assume(parent.notNode())}),
      parent[0],
      !x));
}
                                                             ...
                                                             Node parent)
{
  ...
  {
    return mkNot(mkResolution(
        mkProof(negated ? ProofRule::NOT_XOR_ELIM1 : PfRule::XOR_ELIM2,
                {assume(negated ? parent.notNode() : Node(parent))}),
        parent[1],
        false));
  }
  return mkNot(
      mkResolution(mkProof(negated ? ProofRule::NOT_XOR_ELIM2 : PfRule::XOR_ELIM1,
                           {assume(negated ? parent.notNode() : Node(parent))}),
                   parent[1],
                   true));
}
                                                             ...
                                                             Node parent)
{
  ...
  {
    return mkNot(mkResolution(
        mkProof(negated ? ProofRule::NOT_XOR_ELIM2 : PfRule::XOR_ELIM2,
                {assume(negated ? parent.notNode() : Node(parent))}),
        parent[0],
        false));
  }
  return mkNot(
      mkResolution(mkProof(negated ? ProofRule::NOT_XOR_ELIM1 : PfRule::XOR_ELIM1,
                           {assume(negated ? parent.notNode() : Node(parent))}),
                   parent[0],
                   true));
}
...
std::shared_ptr<ProofNode> ProofCircuitPropagatorBackward::iteC(bool c)
{
  ...
  {
    return mkResolution(
        mkProof(c ? ProofRule::ITE_ELIM1 : PfRule::ITE_ELIM2, {assume(d_parent)}),
        d_parent[0],
        !c);
  }
  return mkNot(
      mkResolution(mkProof(c ? ProofRule::NOT_ITE_ELIM1 : PfRule::NOT_ITE_ELIM2,
                           {assume(d_parent.notNode())}),
                   d_parent[0],
                   !c));
}
...
std::shared_ptr<ProofNode> ProofCircuitPropagatorBackward::iteIsCase(unsigned c)
{
  ...
  if (d_parentAssignment)
  {
    return mkResolution(mkProof(c == 0 ? ProofRule::ITE_ELIM1 : PfRule::ITE_ELIM2,
                                {assume(d_parent)}),
                        d_parent[c + 1],
                        true);
  }
  return mkResolution(
      mkProof(c == 0 ? ProofRule::NOT_ITE_ELIM1 : PfRule::NOT_ITE_ELIM2,
              {assume(d_parent.notNode())}),
      d_parent[c + 1],
      false);
}
...
std::shared_ptr<ProofNode> ProofCircuitPropagatorForward::orOneTrue()
{
  ...
  auto it = std::find(d_parent.begin(), d_parent.end(), d_child);
  return mkNot(mkResolution(
      mkProof(ProofRule::CNF_OR_NEG, {}, {d_parent, mkInt(it - d_parent.begin())}),
      d_child,
      false));
}
...
std::shared_ptr<ProofNode> ProofCircuitPropagatorForward::iteEvalThen(bool x)
{
  ...
  return mkCResolution(
      mkProof(x ? ProofRule::CNF_ITE_NEG1 : PfRule::CNF_ITE_POS1, {}, {d_parent}),
...
std::shared_ptr<ProofNode> ProofCircuitPropagatorForward::iteEvalElse(bool y)
{
  ...
  return mkCResolution(
      mkProof(y ? ProofRule::CNF_ITE_NEG2 : PfRule::CNF_ITE_POS2, {}, {d_parent}),
...
std::shared_ptr<ProofNode> ProofCircuitPropagatorForward::eqEval(bool x, bool y)
{
  ...
  {
    return mkCResolution(
        mkProof(x ? ProofRule::CNF_EQUIV_NEG2 : PfRule::CNF_EQUIV_NEG1,
                {},
                {d_parent}),
        {d_parent[0], d_parent[1]},
        {!x, !y});
  ...
  return mkCResolution(
      mkProof(
          x ? ProofRule::CNF_EQUIV_POS1 : PfRule::CNF_EQUIV_POS2, {}, {d_parent}),
      {d_parent[0], d_parent[1]},
      {!x, !y});
}
    ...
    bool premise, bool conclusion)
{
  ...
  {
    return mkResolution(
        mkProof(ProofRule::CNF_IMPLIES_NEG1, {}, {d_parent}), d_parent[0], true);
  }
  if (conclusion)
  {
    return mkResolution(
        mkProof(ProofRule::CNF_IMPLIES_NEG2, {}, {d_parent}), d_parent[1], false);
  }
