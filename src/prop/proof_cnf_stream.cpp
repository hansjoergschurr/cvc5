...
void ProofCnfStream::convertAndAssertXor(TNode node, bool negated)
{
  ...
  else
  {
    ...
    if (added)
    {
      ...
      d_proof.addStep(clauseNode, ProofRule::NOT_XOR_ELIM2, {node.notNode()}, {});
      Trace("cnf")
    ...
    if (added)
    {
      ...
      d_proof.addStep(clauseNode, ProofRule::NOT_XOR_ELIM1, {node.notNode()}, {});
      Trace("cnf")
...
void ProofCnfStream::convertAndAssertImplies(TNode node, bool negated)
{
  ...
  else
  {
    ...
    d_proof.addStep(node[0], ProofRule::NOT_IMPLIES_ELIM1, {node.notNode()}, {});
    Trace("cnf")
...
void ProofCnfStream::convertAndAssertIte(TNode node, bool negated)
{
  ...
  if (added)
  {
    ...
    else
    {
      Node clauseNode =
          nm->mkNode(kind::OR, node[0].notNode(), node[1].notNode());
      d_proof.addStep(clauseNode, ProofRule::NOT_ITE_ELIM1, {node.notNode()}, {});
      Trace("cnf")
  ...
  if (added)
  {
    ...
    else
    {
      ...
      d_proof.addStep(clauseNode, ProofRule::NOT_ITE_ELIM2, {node.notNode()}, {});
      Trace("cnf")
